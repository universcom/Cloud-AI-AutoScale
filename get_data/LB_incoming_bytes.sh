...
measures_id=`gnocchi resource show $LB_id -c metrics -f yaml| grep -w "network.services.lb.incoming.bytes" | cut -d ':' -f 2`
value=`gnocchi measures show $measures_id --granularity 1 | tail -n 2 | head -n 1 | cut -d "|" -f 4 | sed 's/ //g'`
if [[ $? -eq "0" ]]
then
	echo `date +"%d-%m-%y-%T"` " | " $value >> LB_incoming_bytes.txt
fi
