...
measures_id=`gnocchi resource show $Res_id -c metrics -f yaml | grep -w "network.incoming.bytes.rate:" | cut -d ":" -f2`
value=`gnocchi measures show $measures_id --granularity 1 | tail -n 2 | head -n 1 | cut -d "|" -f 4 | sed 's/ //g'`
if [[ $? -eq "0" ]]
then
        echo `date +"%d-%m-%y-%T"` " | " $value >> instance_incoming_bytes_rate.txt
fi
